...
DROP FUNCTION IF EXISTS mq_checkpoint;
DROP FUNCTION IF EXISTS mq_keep_alive;
DROP FUNCTION IF EXISTS mq_delete;
DROP FUNCTION IF EXISTS mq_commit;
DROP FUNCTION IF EXISTS mq_insert;
DROP FUNCTION IF EXISTS mq_poll;
DROP FUNCTION IF EXISTS mq_active_channels;
DROP FUNCTION IF EXISTS mq_latest_message;
DROP TABLE IF EXISTS mq_payloads;
DROP TABLE IF EXISTS mq_msgs;
DROP FUNCTION IF EXISTS mq_uuid_exists;
DROP TYPE IF EXISTS mq_new_t;
...
DROP TRIGGER IF EXISTS trigger_delete_old_rows_bg_status ON bg_status;
CREATE TRIGGER trigger_delete_old_rows_bg_status
    AFTER INSERT ON bg_status
    EXECUTE PROCEDURE delete_old_rows();


CREATE OR REPLACE FUNCTION update_timestamp_bg_status() RETURNS TRIGGER
    LANGUAGE plpgsql
    AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trigger_update_timestamp_bg_status on bg_status;
CREATE TRIGGER trigger_update_timestamp_bg_status
    BEFORE UPDATE ON bg_status
    EXECUTE PROCEDURE update_timestamp_bg_status();
